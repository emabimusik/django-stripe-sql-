...
def checkout(request):
    ...
    # coupon dictionary
    coupons = {'easter': 25, 'welcome': 15}

    if request.method == 'POST':
        stripe_customer =stripe.Customer.create(email=request.user.email, source=request.POST['stripeToken'])
        plan = 'plan_EkGXtvy8WkBVA7'
        if request.POST['plan'] == 'yearly':
            plan = 'plan_EkGZg1tLFAtmOZ'
        ...
        if request.method == 'GET' and 'coupon' in request.GET:
            if request.GET['coupon'].lower() in coupons:
                ...
                coupon_kroner = str(coupon_price)[:-2] + ',' + str(coupon_price)[-2:]
                final_kroner = str(price)[:-2] + ','+ str(price)[-2:]
        return render(request, 'plans/checkout.html',  {'plan':plan, 'coupon': coupon,'price': price,
        'og_kroner': og_kroner, 'coupon_kroner': coupon_kroner, 'final_kroner': final_kroner})
